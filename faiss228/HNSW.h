...
struct HNSW {
  ...
  /// hot_hubs and its heat degrees,first:id,second:heatDegree
  std::unordered_map<idx_t,int> globleHeatDegrees;

  /// 每个点的入度
  std::unordered_map<idx_t,int> indegrees;

  long long globle_cnt=0LL; // 存放上层作为最近邻的总点数 
  long long globle_sum=0LL; // 存放插入过程中最近邻总点数

  long long globle_cnt2=0LL; // 存放上层作为最近邻的总点数 
  long long globle_sum2=0LL; // 存放插入过程中最近邻总点数
  ...
  */
  ...
  // add_reverse_link 3
  void add_reverse_link_by_indegrees(int nb_reverse_nbs_level,std::vector<idx_t>& new_neibor,
                DistanceComputer& dis,idx_t hot_hub,size_t n);
  // 按照入度从小到大，出度从大到小排序选择反向邻居
  void add_reverse_link_by_indegrees_and_outdegrees(int nb_reverse_nbs_level,std::vector<idx_t>& new_neibor,
                DistanceComputer& dis,idx_t hot_hub,size_t n);
...
// 统计找到访问点需要的访问点个数
std::priority_queue<Node> search_from_find_ndis(
  const Node& node,
  DistanceComputer& qdis,
  int ef,
...
void search_ndis(
  ...
  // 热点索引不访问热点反向邻居
  std::priority_queue<HNSW::Node> search_from_candidate_unbounded_hot_hubs_enhence_v2(
        const Node& node,
        DistanceComputer& qdis,
        int ef,
        VisitedTable* vt,
  ...
  /// 热点不访问新增热点反向邻居
  void search_with_hot_hubs_enhence_v2(
          DistanceComputer& qdis,
          int k,
          idx_t* I,
          float* D,
