...
void hnsw_add_vertices(IndexHNSW &index_hnsw,
    ...
    int max_level = hnsw.prepare_level_tab(n, preset_levels);
/*    std::ofstream out("./hubsID1.csv",std::ofstream::app);      
...
// #pragma omp for
        for(idx_t i = 0; i < n; i++) {
            ...
            { 
                ...
                hnsw.search_custom(*dis, k, idxi, simi,vt,search_mode);
            }
...
void IndexHNSW::search_from_hubs_to_nbs () const

{

    {
        ...
        ScopeDeleter1<DistanceComputer> del(dis);
// #pragma omp for
        int nlevels = hnsw.hot_hubs.size(); 
        std::map<int,int> stepsM;
        int sum=0;
        printf("ok\n");
        for(int i = 0; i < nlevels; i++) {
            for (auto a : hnsw.hot_hubs[i])
            {
                auto id = a.first;
                auto nbs = a.second;
                sum+=nbs.size();
                ...
                {
                    VisitedTable vt (ntotal);
                    hnsw.search_from_hubs_to_nbs(std::make_pair(dis->symmetric_dis(id,b),id),b,
                        *dis,1000,vt,stepsM);
                    vt.advance();
                }
            }
        ...
        printf("%d\n",sum);
            // 将距离保存到文件
        std::ofstream out("./search_nsteps.csv",std::ofstream::app);
...
// 输出全局热点
void IndexHNSW::out_HubsIDs_and_its_reverse_nbs(){

    // 输出热点id
    std::ofstream out("./out_data/globleHubsIDS.csv",std::ofstream::app);
    int HbsmpLen = hnsw.hot_hubs.size();
    for(int i=0;i<HbsmpLen;i++){
        for (auto a : hnsw.hot_hubs[i])
        {
...
void addHubsToHubsWithRandom(HNSW& hnsw,DistanceComputer& dis){
    ...
    /*
    std::vector<std::vector<float>> diss(cnt,std::vector<float>(cnt,0));
    // 寻找每个热点的最近邻
    for (int i = 0; i < cnt; ++i)
    {   
        for (int j = 0; j < cnt; ++j)
        {
            if(i!=j){
                diss[i][j] = dis.symmetric_dis(i,j);
            }
        }
    }
    for (int i = 0; i < cnt; ++i)
    {
        std::sort(diss[i].begin(),diss[i].end());
    }
    */
    ...
    for(int i=0;i<HbsmpLen;i++){
        for (auto a : hnsw.hot_hubs[i])
        {
            std::vector<std::pair<float,idx_t>> diss;
            for (auto b : vec)
            {
                if (a.first!=b)
                {
                    diss.emplace_back(dis.symmetric_dis(a.first,b),b);
                }
            }
            std::sort(diss.begin(),diss.end());        
            for(int j=0;j<20;j++){
                a.second.insert(a.second.begin(),diss[j].second);           
            }
        }
    }
}
...
void IndexHNSW::combine_index_with_hot_hubs_enhence(idx_t n,int len_ratios,
        ...
        int find_neighbors_mode, const int* nb_nbors_per_level,idx_t* I){
    // 每个点对应的类别
    std::vector<idx_t> clsf(n);
    for (int i = 0; i < n; ++i)
    {
        clsf[i]=I[i];
    }
    ...
    hnsw.find_hot_hubs_enhence(hnsw.hot_hubs,n,ratios,find_hubs_mode,
            find_neighbors_mode,clsf);    
    // disHToRN(); // 统计热点到其反向邻居的原始距离
    printf("未裁之前上层占最近邻数：%lld\t,未裁之前总共占最近邻数：%lld\t,未裁之前总共占最近邻数：%f\t,裁后上层占最近邻数：%lld\t,裁后总共占最近邻数：%lld\t,裁后总共占最近邻数：%f\n,",
        hnsw.globle_cnt,hnsw.globle_sum,
        hnsw.globle_cnt*1.0/hnsw.globle_sum, 
        hnsw.globle_cnt2,hnsw.globle_sum2,
        hnsw.globle_cnt2*1.0/hnsw.globle_sum2);
    // 距离计算器
    DistanceComputer* dis = get_distance_computer(storage);
    ScopeDeleter1<DistanceComputer> del(dis);

    // out_HubsIDs(hnsw); // 将热点输出
    // addHubsToHubsWithRandom(hnsw,*dis);// 热点随机添加热点反向邻居

    //hnsw.add_links_to_hubs(hnsw.hot_hubs,n); // 将反向边连向热点(随机选取热点邻居)
...
void IndexHNSW::write_index_to_CSV(){
    ...
    std::ofstream out("./out_data/g.csv",std::ofstream::app);
    for(auto a:G){
        out<<a.first<<",";
    ...
    for(auto a:G){
        out1<<a.first<<",";
        out1<<a.second<<std::endl;
    }
    out1.close();     
}


void IndexHNSW::out_indegrees(){
    idx_t n = hnsw.levels.size();
    std::map<idx_t,idx_t> ma;
    ...
    std::ofstream out("./out_data/out_indegrees.csv",std::ofstream::app);
    for(auto a:ma){
        out<<a.first<<",";
        out<<a.second<<std::endl;
    }
...
void IndexHNSW::out_indegrees2(){
    ...
    std::ofstream out("./out_data/out_indegrees_addHubs.csv",std::ofstream::app);
    for(auto a:ma){
        out<<a.first<<",";
        out<<a.second<<std::endl;
    }
...
void IndexHNSW::indegreesDistr(){
    std::map<int,int> indegreesN;
    idx_t n = hnsw.offsets.size();
    ...
    std::ofstream out("./out_data/indegreesDistr.csv",std::ofstream::app);
    for(auto a:indegreesN){
        out<<a.first<<",";
    }
    out<<std::endl;
    for(auto a:indegreesN){
...
void IndexHNSW::level1_hubs(){
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {   
            ...
            if ( hnsw.globleHeatDegrees.find(i) != hnsw.globleHeatDegrees.end() )
            {
                sum1++;
                vct.emplace_back(i);
            }
    ...
    // 将距离保存到文件
    std::ofstream out("./out_data/HubsID.csv",std::ofstream::app);
    ...
    printf("\n");
    int gs = hnsw.globleHeatDegrees.size();
    float ratio = 1.0*sum1/(1.0*gs);
    float ratio2 = 1.0*sum1/(1.0*sum);
    printf("1层热点总数：%d\t,热点总数：%d\t,1层热点占总热点比例：%f\t,1层总点数：%d\t,热点占1层总点数比例：%f\n",
        sum1,gs,ratio,sum,ratio2);
}



// 热点到原始图中反向邻居的距离统计
void IndexHNSW::disHToRN(){
    auto& hbs = hnsw.hot_hubs;
    int HbsLen = hbs.size();
    std::map<int ,int > disLen;
    for (int i = 1; i < HbsLen; ++i)
    {
        // i层热点
        for (auto a:hbs[1])
        {
            ...
            for(auto b:nbs){
                ...
                std::queue<std::pair<idx_t,int>> q;
                q.emplace(id,0);
                vis.insert(id);
                int cnt = 0;
                while(!q.empty()){
                    ...
                    int flag=0;
                    auto fid = q.front().first;
                    auto slevel = q.front().second;
                    // printf("%d\n",slevel);
                    if(slevel>20){
                        disLen[101]++;
                        break;
                    ...
                    q.pop();
                    size_t begin, end;
                    hnsw.neighbor_range(fid, 0, &begin, &end);
                    for (size_t j = begin; j < end; j++) {
                        // printf("levels.size():%d\n",hnsw.levels.size());
                        auto cur = hnsw.neighbors[j];
                        if (cur<0)
                            break;
                        else if (cur==b)
                        {
                        ...
                        {
                            q.emplace(cur,slevel+1);vis.insert(cur);
                        }     
                    } 
                    if (flag==-1)
                      {
...
// 热点索引，但是不访问热点邻居
void IndexHNSW::search_with_hot_hubs_enhence_v2(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
...
//#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                dis->set_query(x + i * d);
                maxheap_heapify(k, simi, idxi);
                hnsw.search_with_hot_hubs_enhence_v2(*dis, k, idxi, simi, vt,xb_size,rng3);
                maxheap_reorder(k, simi, idxi);
...

{
    ...
    size_t n1 = 0, n2 = 0, n3 = 0, ndis = 0, nreorder = 0;
    idx_t check_period = 10000;
    // each hot_hub visited times in searching phase
    std::unordered_map<idx_t,int> fqc;
    for (idx_t i0 = 0; i0 < n; i0 += check_period) {
        idx_t i1 = std::min(i0 + check_period, n);
...
//#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                // printf("ok4\n");
                maxheap_heapify(k, simi, idxi);
                hnsw.search_with_hot_hubs_enhence_random(*dis, k, idxi, simi, vt,xb_size,rng3,fqc);
                // printf("ok5\n");
                maxheap_reorder(k, simi, idxi);
                // 输出热点热度及其访问频率
            }
    ...
    std::vector<std::pair<int,idx_t>> idss;
    for(auto a : fqc){
        auto id = a.first;
        auto ff = a.second;
        // 根据id 获取热度，存入idss ，根据id 获取访问频率
        // std::cout<<hnsw.globleHeatDegrees.at(id)<<"::"<<fqc[id]<<std::endl;
        idss.emplace_back(hnsw.globleHeatDegrees.at(id),fqc[id]);
    }   
    std::sort(idss.begin(),idss.end());
    std::ofstream out("./fqc.csv",std::ofstream::app);
    int nums=idss.size();
    ...
    std::ofstream out1("./fqc1.csv",std::ofstream::app);
    std::vector<std::pair<int,idx_t>> tt;
    for(auto a : fqc){
        auto id = a.first;
        auto ff = a.second;
        tt.emplace_back(id,ff);
    }  
    std::sort(tt.begin(),tt.end(),[&](std::pair<int,idx_t> l,std::pair<int,idx_t> r){
...
// 统计找到gt 需要的ndis
void IndexHNSW::search_with_ndis(
        idx_t n,
        const float* x,
        idx_t k,
        float* distances,
        idx_t* labels,size_t xb_size,const idx_t* I) const
...
//#pragma omp for reduction(+ : n1, n2, n3, ndis, nreorder)
            for (idx_t i = i0; i < i1; i++) {
                ...
                // printf("ok4\n");
                maxheap_heapify(k, simi, idxi);
                hnsw.search_ndis(*dis, k, idxi, simi, vt,xb_size,rng3,ndiss,gt[i]);
                // printf("ok5\n");
                maxheap_reorder(k, simi, idxi);
/*                printf("%f\t,%f\t,%d\t,%d\n",
                                    simi[0],simi[1],
    ...
    std::ofstream out("./out_data/ndisToGt.csv",std::ofstream::app);      
    for (int i = 0; i < (int)ndiss.size(); ++i){
      out<<ndiss[i]<<std::endl;
    }
    out<<std::endl;
    out.close();

    std::ofstream out1("./out_data/gt.csv",std::ofstream::app);      
    for (int i = 0; i < n; ++i){
      out1<<gt[i]<<std::endl;
    }
    out1<<std::endl;
