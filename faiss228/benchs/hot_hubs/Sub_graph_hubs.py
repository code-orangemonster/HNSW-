...
def createIndex(xb,nbs): 
    index = faiss.IndexHNSWFlat(xb.shape[1], nbs)
...
for i in range(5):
    ...
    print(sub_xb.shape)
    # 构建子索引放入indexList
    sub_index = createIndex(sub_xb,10)
...
idx4 = np.array(indexScript[3])
idx5 = np.array(indexScript[4])
print(idx1[0:5])
print(idx1.shape)
print(ratios.shape)
...
index.enhence_index_with_subIndex_hubs(n,2,faiss.swig_ptr(ratios),0,0,
    ...
    indexList[2],indexList[3],indexList[4],
    faiss.swig_ptr(idx1),
    faiss.swig_ptr(idx2),
    faiss.swig_ptr(idx3),
    faiss.swig_ptr(idx4),
